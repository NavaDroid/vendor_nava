...
func init() {
	android.RegisterModuleType("nava_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = navaExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = navaExpandVariables(ctx, dep_file)
	...
	cmd := navaExpandVariables(ctx, String(g.properties.Cmd))
